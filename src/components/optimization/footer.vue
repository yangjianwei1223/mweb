<template>
  <div class="seatfooter">
    ...
    <a href="javascript:;" @click="isopensku" class="act-item buybtn" v-if="footdata.SaleType===3">立即租赁</a>
    <template v-else-if="footdata.SaleType===1">
      <a @click="isopensku('cart')" class="act-item cartbtn">加入购物车</a>
      <a @click="isopensku" class="act-item buybtn">立即购买</a>
    </template>
    <template v-else>
      <template v-if="footdata.Status === 2">
        <a class="act-item buybtn changeinvalid">已下架</a>
      </template>
      <template v-else-if="footdata.Status === 3">
        <a class="act-item buybtn changeinvalid">已售罄</a>
      </template>
      <template v-else>
        <a @click="isopensku('cart')" class="act-item cartbtn">加入购物车</a>
        <a @click="isopensku" class="act-item buybtn">立即购买</a>
